...
### Amendment 7 (post-/done, from /do) — Feature-parity drift: restore /drive defaults

**Trigger**: User — "Make sure defaults are same as the removed drive like not 30 min between ci checks but less etc. In general ensure we didn't lose or change much features." Investigation against /drive's source at the parent of the removal commit (`869d99d^`) confirmed two drifts and one no-op:

| Knob | /drive default (verified at 869d99d^) | Current state | Action |
|------|---------------------------------------|---------------|--------|
| Tick / CI re-check cadence | `--interval 15m` (drive SKILL.md line 23; range 15m–24h) | PR_LIFECYCLE.md line 39: "CI-poll cadence — default `timeout: 30m`" (number drifted up; also conflates cadence with wall-clock timeout) | Restore 15m between-check cadence; separate cadence from AC `timeout:` semantics |
| CI retrigger cap | `CI_RETRIGGER_CAP = 10 per run` (drive/references/platforms/github.md line 15) | 3 per failing check — in `agents/github-pr-lifecycle.md` line 78, `tasks/PR_LIFECYCLE.md` line 41 + 72, AC-1.1 description, AC-1.1 verifier prompt clause 6 | Restore 10 per failing check (matches /drive's CI_RETRIGGER_CAP headroom; per-check granularity is the current model's shape and is close enough to per-run for the same intent) |
| Stale thread window | 30 min (`STALE_THREAD_WINDOW`) | ~30 min in agent line 80 | Match ✓ no change |
| Tick budget `--max-ticks` | 100 | n/a — /do has no tick concept | n/a — wall-clock `verify.timeout:` replaces it |

**Why this drifted past PG-4.** PG-4 (Establish behavior contract before removal — for every /drive feature listed in the feature parity mapping, confirm the new home is present and operational) is the gate that should have caught these. The parity mapping in `/tmp/define-discovery-20260512-053955.md` evidently treated "agent has a retrigger-cap" and "agent polls CI" as the carry-overs without auditing the specific default values against /drive's actual numbers. Logging here as a retroactive PG-4 miss; corrective verification approach baked into Amendment 7's AC verifier prompt below.

**Rationale**: feature parity. The user's request explicitly named "/drive's defaults" as the target. Lowering cadence to 15m matches /drive's `--interval` default; raising retrigger cap to 10 matches /drive's `CI_RETRIGGER_CAP` for transient infra failures. The /do collapse traded the tick loop for a single in-session wait pattern; the cadence and cap numbers are the only continuity-relevant /drive knobs that survive that shape change.

**Changes**:

- **AC-1.1 description** — `default cap 3` → `default cap 10` (matches /drive's `CI_RETRIGGER_CAP`).
- **AC-1.1 verifier prompt clause 6** — `default cap 3, escalate after cap` → `default cap 10, escalate after cap`. The prompt-reviewer pass will catch any lingering "3" in the agent body.

**/do file changes** (executed in /do, not /define):

- `claude-plugins/manifest-dev/agents/github-pr-lifecycle.md` (line 78): `default 3 per failing check across this agent's lifetime for the PR` → `default 10 per failing check across this agent's lifetime for the PR (matches /drive's CI_RETRIGGER_CAP for parity)`.
- `claude-plugins/manifest-dev/skills/define/tasks/PR_LIFECYCLE.md` (line 39): rewrite "CI-poll cadence" entry to distinguish (a) **between-check cadence** default `15m` (matches /drive's `--interval` default — the agent waits roughly this long between CI re-checks when a CI suite is still running) from (b) **AC `timeout:`** which is the wall-clock upper bound for the whole criterion (1d remains the suggested default for approval-wait-dominated lifecycles; surface a longer override when CI suites legitimately exceed). The 30m timeout was arbitrary and had no /drive precedent.
- `claude-plugins/manifest-dev/skills/define/tasks/PR_LIFECYCLE.md` (line 41): `agent default is 3 retriggers per failing CI check per AC lifetime` → `agent default is 10 retriggers per failing CI check per AC lifetime (matches /drive's CI_RETRIGGER_CAP for parity)`.
- `claude-plugins/manifest-dev/skills/define/tasks/PR_LIFECYCLE.md` (line 72): `Default cap of 3 may starve a genuinely flaky job` → `Default cap of 10 may still starve a chronically flaky job`.
- `dist/{gemini,opencode,codex}/agents/github-pr-lifecycle.{md,toml}` and `dist/{gemini,opencode,codex}/skills/define/tasks/PR_LIFECYCLE.md` — re-sync.

**Unchanged (load-bearing, kept):**
- Stale thread window — 30 min in both /drive and current agent.
- All other ACs, PGs, INV-Gs.
- The /drive features that legitimately retired (tick loop, `--max-ticks`, `--interval`, inline-fallback chunked-sleep) — the new single-session /do model replaces them by design; not parity drifts.
- Defense-in-depth layers (INV-G8 merge-button grep + github-pr-lifecycle.md Hard prohibitions) — unchanged.

**/do scope hint**: D1 only (agent + PR_LIFECYCLE.md task file). Recommend `--scope D1`.
