...
- **Mental Model:**
  - *Verifier-hint protocol* — criteria-checker returns instant PASS or rich FAIL whose body is free-form text describing what to do next (sleep N minutes, retrigger CI, reply to thread X, push update Y, fix code, amend manifest). /do reads the body with LLM judgment and dispatches the appropriate action. No new schema — /do is an LLM that can read English.
...
- [PG-7] Consistent action vocabulary across agent, verifier, and /do prompts — the canonical action set is `sleep`, `fix-code`, `retrigger-ci`, `reply-thread`, `push-update`, `amend-manifest`. `github-pr-lifecycle` agent emits hints in this vocabulary; non-lifecycle verifier prompts use the same words when emitting hints directly. The vocabulary is a closed set: `merge-pr` (or any merge-button verb) is explicitly NOT a member — terminal is "mergeable" not "merged". /do SKILL.md, criteria-checker.md, and github-pr-lifecycle.md must agree on the list verbatim.
...
- [AC-3.3] /do SKILL.md adds a "Hint Dispatch" section (or equivalent) listing the action vocabulary, the dispatch mapping (each action → tool invocation pattern: `sleep` shell, edit tool, `gh run rerun`, `gh pr comment`, `gh pr edit`/`git push`, `/define --amend --from-do`), and how /do parses free-form hints using LLM judgment. Notes that lifecycle ACs route through agent-emitted hints; non-lifecycle ACs may emit hints directly. Existing Mid-Execution Amendment section is updated to note that verifier-emitted amend hints also route via Self-Amendment. Explicit statement: `merge-pr` is NOT a supported action; /do does NOT invoke `gh pr merge`.
  ...
  verify:
    ...
    agent: prompt-reviewer
    model: inherit
    prompt: "Review claude-plugins/manifest-dev/skills/do/SKILL.md for the new Hint Dispatch behavior. Verify: (1) section names the supported action vocabulary (sleep, fix-code, retrigger-ci, reply-thread, push-update, amend-manifest); (2) maps each action to a concrete tool invocation pattern; (3) describes parsing as LLM-judgment based (no rigid schema); (4) Mid-Execution Amendment section updated to note verifier-emitted amend hints also route via Self-Amendment / /define --amend --from-do; (5) lifecycle ACs route through agent-emitted hints (github-pr-lifecycle), non-lifecycle ACs may emit hints directly; (6) explicit prohibition — merge-pr is NOT a supported action, /do does NOT invoke gh pr merge under any path. Threshold: no MEDIUM+ findings."
  ```

- [AC-3.4] Execution-mode files (`thorough.md`, `balanced.md`, `efficient.md`) document action-aware fix-cap semantics: only `fix-code` actions increment the per-phase fix-verify counter; `sleep`, `retrigger-ci`, `reply-thread`, `push-update`, `amend-manifest` are each named explicitly as non-counting actions. Per-AC `verify.timeout:` is the wall-clock cap that prevents lifecycle-AC runaway.
  ```yaml
  verify:
    method: codebase
    prompt: "In claude-plugins/manifest-dev/skills/do/references/execution-modes/{thorough,balanced,efficient}.md: verify each file's Fix-Verify Loops (or equivalent) section explicitly says the cap counts fix-code actions only; sleep, retrigger-ci, reply-thread, push-update, AND amend-manifest are each named (each action label appears in the text) as non-counting. Verify a cross-reference to per-AC verify.timeout: for lifecycle waits."
  ```
