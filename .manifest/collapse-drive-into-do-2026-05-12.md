...
- [AC-1.1] `claude-plugins/manifest-dev/agents/github-pr-lifecycle.md` exists with valid frontmatter (name, description suitable for semantic match by /define, tools list including Bash + Read + necessary GitHub MCP tools for PR/CI/comment inspection, no Write/Edit since the agent is read-only). Body documents: purpose, expected invocation inputs (PR URL, branch, optional customization overlay from the AC's verify.prompt), state inspection commands (`gh pr view --json mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,reviewThreads,...`), decision logic for the canonical gates (PR exists, CI green, threads addressed, PR description in sync, PR mergeable composite), output format (PASS or FAIL with rich hint body), hint vocabulary the agent may emit (`sleep`, `fix-code`, `retrigger-ci`, `reply-thread`, `push-update`, `out-of-scope`), retrigger-cap logic (read execution log, default cap 3, escalate phrasing after cap), thread classification rules (bot vs human, in-scope vs out-of-scope), and explicit hard prohibitions: agent NEVER emits `merge-pr`, NEVER invokes `gh pr merge`, terminal is "mergeable" not "merged"; agent NEVER references `/define`, manifest amendment, Self-Amendment, or any workflow-routing concept — the agent reports findings, /do owns dispatch decisions. The agent's hint vocabulary describes *findings* (e.g., "out-of-scope" = "the situation is beyond the current scope"), not workflow actions; /do internally maps findings to workflow steps.
  ...
  verify:
    ...
    agent: prompt-reviewer
    model: inherit
    prompt: "Review claude-plugins/manifest-dev/agents/github-pr-lifecycle.md for prompt-engineering correctness and structural completeness. Verify: (1) frontmatter — name = github-pr-lifecycle, description semantically supports matching the agent for PR/lifecycle/mergeable concepts AND signals that the agent is steerable through the invoking AC's verify.prompt (so callers and readers understand customization is first-class), tools list includes Bash and Read and explicitly excludes Write/Edit (the agent is read-only inspection); (2) body documents invocation inputs (PR URL, branch, user-supplied steering prompt from the AC's verify.prompt); (3) state inspection details `gh pr view --json mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,reviewThreads` (or equivalent MCP calls) as the single source of truth; (4) decision logic for the canonical gates is documented (PR exists / CI green / threads addressed / PR description in sync / PR mergeable composite); (5) output format documents PASS-or-FAIL-with-rich-hint; (6) hint vocabulary explicitly lists sleep/fix-code/retrigger-ci/reply-thread/push-update/out-of-scope (and NO others — `amend-manifest` and `merge-pr` must NOT appear as emittable labels); (7) retrigger-cap logic documented (read execution log, default cap 3, escalate after cap); (8) thread classification documented (bot vs human, in-scope vs out-of-scope); (9) hard prohibitions explicitly stated — agent NEVER emits merge-pr, NEVER invokes gh pr merge, terminal is mergeable not merged; (10) **steerability** documented as a first-class property — the agent reads the invoking AC's verify.prompt as the user's steering input, layered ADDITIVELY on baseline behavior. Empty steering prompt → baseline only; user-supplied stricter constraint (e.g., 'retrigger cap = 5' when baseline is 3) narrows-wins (overlay precedence for that constraint); user can add gates beyond the canonical set (custom labels, named approvers, known-flaky CI handling). The word 'steerable' or an obvious equivalent ('configurable via the invoking prompt', 'tunable through verify.prompt') must appear so readers see customization as first-class, not buried implementation detail; (11) **workflow decoupling** — the agent body must NOT reference `/define`, `/define --amend`, manifest amendment, Self-Amendment, AMENDMENT_MODE, or any other workflow-routing concept. The agent reports findings; /do owns dispatch decisions. The word `/define` (and the phrase `manifest amendment` in any form) must not appear anywhere in the agent body. The `out-of-scope` hint label is described as a finding (e.g., 'reviewer requested change beyond current scope'), never as a workflow action. Threshold: no MEDIUM+ findings."
  ```
