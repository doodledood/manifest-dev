...
- **Mental Model:**
  - *Verifier-hint protocol* — criteria-checker returns instant PASS or rich FAIL whose body is free-form text describing what to do next (sleep N minutes, retrigger CI, reply to thread X, push update Y, fix code, surface out-of-scope finding). /do reads the body with LLM judgment and dispatches the appropriate action — including mapping `out-of-scope` findings to Self-Amendment / `/define --amend` internally. No new schema — /do is an LLM that can read English. **Agent-side vocabulary is decoupled from /do's dispatch decisions**: the agent emits findings (out-of-scope, etc.); /do decides how to route them. The agent never references /define or any workflow concept.
  - *Action-aware budget* — execution-modes per-phase fix-cap only increments on code-fix actions; sleep/retrigger/reply/push/amend don't burn the fix budget. Per-AC `verify.timeout:` provides wall-clock cap for runaway protection on lifecycle ACs (e.g., 30m CI-poll, 1d approval-wait — approvals are the slowest gate, often human-paced).
...
- [PG-7] Consistent hint vocabulary across agent, verifier, and /do prompts — the canonical set is `sleep`, `fix-code`, `retrigger-ci`, `reply-thread`, `push-update`, `out-of-scope`. `github-pr-lifecycle` agent emits hints in this vocabulary; non-lifecycle verifier prompts use the same words when emitting hints directly. The vocabulary is **agent-side findings**, not /do dispatch actions — /do owns the mapping from finding to workflow (e.g., `out-of-scope` → Self-Amendment / `/define --amend`). The vocabulary is a closed set: `merge-pr` (or any merge-button verb) is explicitly NOT a member — terminal is "mergeable" not "merged"; `amend-manifest` is also NOT a member — replaced by `out-of-scope` so the agent does not name /define. /do SKILL.md, criteria-checker.md, and github-pr-lifecycle.md must agree on the list verbatim.
...
- [AC-1.1] `claude-plugins/manifest-dev/agents/github-pr-lifecycle.md` exists with valid frontmatter (name, description suitable for semantic match by /define, tools list including Bash + Read + necessary GitHub MCP tools for PR/CI/comment inspection, no Write/Edit since the agent is read-only). Body documents: purpose, expected invocation inputs (PR URL, branch, optional customization overlay from the AC's verify.prompt), state inspection commands (`gh pr view --json mergeable,mergeStateStatus,reviewDecision,statusCheckRollup,reviewThreads,...`), decision logic for the canonical gates (PR exists, CI green, threads addressed, PR description in sync, PR mergeable composite), output format (PASS or FAIL with rich hint body), hint vocabulary the agent may emit (`sleep`, `fix-code`, `retrigger-ci`, `reply-thread`, `push-update`, `out-of-scope`), retrigger-cap logic (read execution log, default cap 3, escalate phrasing after cap), thread classification rules (bot vs human, in-scope vs out-of-scope), and explicit hard prohibitions: agent NEVER emits `merge-pr`, NEVER invokes `gh pr merge`, terminal is "mergeable" not "merged"; agent NEVER references `/define`, manifest amendment, Self-Amendment, or any workflow-routing concept — the agent reports findings, /do owns dispatch decisions. The agent's hint vocabulary describes *findings* (e.g., "out-of-scope" = "the situation is beyond the current scope"), not workflow actions; /do internally maps findings to workflow steps.
  ```yaml
  verify:
    method: subagent
    agent: prompt-reviewer
    model: inherit
    prompt: "Review claude-plugins/manifest-dev/agents/github-pr-lifecycle.md for prompt-engineering correctness and structural completeness. Verify: (1) frontmatter — name = github-pr-lifecycle, description semantically supports matching the agent for PR/lifecycle/mergeable concepts AND signals that the agent is steerable through the caller's invoking prompt (so callers and readers understand customization is first-class), tools list includes Bash and Read and explicitly excludes Write/Edit (the agent is read-only inspection); (2) body documents invocation inputs (PR URL, branch, optional steering input passed by the caller) — described agent-neutrally, not as `the invoking AC's verify.prompt:`; (3) the agent names the kinds of state it must reach to evaluate the canonical gates (mergeable composite, CI status, review threads, PR description, etc.) WITHOUT prescribing specific gh CLI commands, JSON field lists, or API surface details — trust the model to know the GitHub API and pick the right calls (verify the prompt does not contain prescriptive `gh pr view --json ...` field-name enumerations or step-by-step API recipes; the agent is goal-shaped, not procedure-shaped); (4) the agent documents WHAT each canonical gate verifies (the goal of the check, e.g., 'PR is mergeable per GitHub's composite state') — NOT exact procedural steps to check it; (5) output format documents PASS-or-FAIL-with-rich-hint; (6) hint vocabulary explicitly lists sleep/fix-code/retrigger-ci/reply-thread/push-update/out-of-scope (and NO others — `amend-manifest` and `merge-pr` must NOT appear as emittable labels); (7) retrigger-cap logic documented agent-neutrally — prior retrigger counts sourced from \"any context made available to this invocation\" without prescribing a specific source format; default cap 3, escalate after cap; (8) thread classification documented as goals (distinguish bot/human, classify intent, identify in-scope/out-of-scope) — NOT as a rigid procedure; (9) hard prohibitions explicitly stated — agent NEVER emits merge-pr, NEVER invokes gh pr merge, terminal is mergeable not merged; (10) **steerability** documented as a first-class property — the agent reads the invoking prompt as the user's steering input, layered ADDITIVELY on baseline behavior. Empty steering prompt → baseline only; user-supplied stricter constraint (e.g., 'retrigger cap = 5' when baseline is 3) narrows-wins (overlay precedence for that constraint); user can add gates beyond the canonical set (custom labels, named approvers, known-flaky CI handling). The word 'steerable' or an obvious equivalent ('configurable via the invoking prompt', 'tunable via the caller's prompt') must appear so readers see customization as first-class, not buried implementation detail; (11) **Portability — agent is workflow-agnostic, runnable by any caller**. Like prompt-reviewer or code-bugs-reviewer can be invoked outside this repo, this agent must be reusable without assuming any specific orchestration. **Banned tokens in the agent body** (verified by case-insensitive grep): `/do`, `/verify`, `/define`, `/escalate`, `/done`, `--scope`, `--amend`, `verify.prompt`, `AC's`, `the invoking AC`, `INV-G`, `manifest amendment`, `Self-Amendment`, `AMENDMENT_MODE`, `execution log`, `memento`, `PR_LIFECYCLE`, `task file`, `deliverable`, `verify-loop`. Generic English use of `manifest` referring to a plan/scope/contract (not the manifest-dev schema) IS allowed — e.g., \"beyond the current manifest's scope\" reads as a finding regardless of which workflow the caller uses. Frontmatter description is exempt only for `tools` list values (e.g., `Bash`, `Read`) being legitimate tool names; banned-token check applies to the body. **Multi-PR composition** in the body MUST be described agent-neutrally: \"the agent handles one PR per invocation; multi-PR composition is the caller's responsibility\" — no naming PR_LIFECYCLE.md, manifest-dev, or any specific task file; (12) **Prompt-engineering hygiene** — the agent prompt follows the principles in claude-plugins/prompt-engineering (WHAT and WHY, not HOW; trust the model on capability). Specifically check for: NO prescriptive HOW (no 'first do X, then do Y' recipes; no `gh pr view --json <specific-field-list>` field enumerations; no step-by-step command sequences). NO capability instructions ('use grep to search', 'read the file', 'parse the JSON', 'split on whitespace'). NO arbitrary number limits without justification ('check 5 commits', 'wait 3 seconds') unless the number is itself an invariant. NO rigid checklists masquerading as guidance. Decision rules ('when X, do Y; otherwise Z') are fine; rigid procedures are not. ABSOLUTES (MUST/NEVER/ALWAYS) are reserved for true invariants — the closed action vocabulary, the hard prohibitions (no merge button, no force-push, no secret exposure), and the workflow-decoupling clause from (11). Steerability semantics may use MUST when describing the additive-overlay invariant. Other content stays goal-shaped: 'The agent inspects state needed to evaluate ...' rather than 'The agent SHALL execute the following commands ...'. Use the prompt-engineering skill or review-prompt skill to evaluate this clause if uncertain. Threshold: no MEDIUM+ findings."
  ```
...
- [AC-3.2] `claude-plugins/manifest-dev/agents/criteria-checker.md` documents the rich-hint output convention — FAIL outputs include actionable, free-form text that /do can interpret as one of: sleep N, fix code, retrigger CI, reply to thread X with Y, push update Y, out-of-scope finding Z. Vocabulary list is present so verifier authors stay consistent. Notes that `github-pr-lifecycle` (and other future platform agents) are the canonical producers for lifecycle hints; non-lifecycle verifiers may emit hints directly using the same vocabulary. The vocabulary names *findings*, not workflow actions — /do internally maps findings to workflow steps (e.g., `out-of-scope` → Self-Amendment).
  ```yaml
  verify:
    method: subagent
    agent: prompt-reviewer
    model: inherit
    prompt: "Review claude-plugins/manifest-dev/agents/criteria-checker.md for prompt quality (no MEDIUM+). Verify the Output Format section documents that FAIL bodies may include actionable hints in free-form English, lists the known hint vocabulary (sleep, fix-code, retrigger-ci, reply-thread, push-update, out-of-scope — and explicitly NOT amend-manifest, NOT merge-pr), names github-pr-lifecycle as the canonical lifecycle producer, and clarifies that non-lifecycle verifiers may emit hints directly using the same vocabulary. The vocabulary names findings, not workflow actions; /do owns mapping findings to workflow steps. Verify clarity on when to emit which hint type. **Light-touch portability**: the agent's overall role statement (the opening section describing its purpose) should be generic enough that the agent could be invoked by callers other than /verify — for example, framed as 'verify a single constraint given a description and an automated method, returning structured PASS/FAIL.' Workflow-internal terms (`/verify`, `INV-G`, `AC-{D}.{N}`, `Manifest`, `Spawned by /verify in parallel`) MAY appear in Input-format and Type-Specific Guidance sections since those describe inputs the caller passes — but the agent's purpose statement should not REQUIRE manifest-dev's workflow. Flag a MEDIUM finding if the opening of the agent ('purpose' or first paragraph) hard-couples to manifest-dev (e.g., 'verify a single criterion FROM A MANIFEST'). Acceptable: 'verify a SINGLE criterion. You are READ-ONLY—check, don't modify' (current opening is fine). Not acceptable: an opening that assumes the caller MUST be /verify or that the input MUST be a manifest criterion."
  ```

- [AC-3.3] /do SKILL.md adds a "Hint Dispatch" section (or equivalent) listing the action vocabulary, the dispatch mapping (each action → tool invocation pattern: `sleep` shell, edit tool, `gh run rerun`, `gh pr comment`, `gh pr edit`/`git push`, `/define --amend --from-do`), and how /do parses free-form hints using LLM judgment. Notes that lifecycle ACs route through agent-emitted hints; non-lifecycle ACs may emit hints directly. Existing Mid-Execution Amendment section is updated to note that verifier-emitted amend hints also route via Self-Amendment. Explicit statement: `merge-pr` is NOT a supported action; /do does NOT invoke `gh pr merge`.
  ```yaml
  verify:
    method: subagent
    agent: prompt-reviewer
    model: inherit
    prompt: "Review claude-plugins/manifest-dev/skills/do/SKILL.md for the Hint Dispatch behavior. Verify: (1) section names the supported hint vocabulary the agent/verifier may emit (sleep, fix-code, retrigger-ci, reply-thread, push-update, out-of-scope — `amend-manifest` and `merge-pr` are NOT in this vocabulary); (2) maps each hint to a concrete tool invocation or workflow step — specifically, `out-of-scope` maps internally to Self-Amendment (`/define --amend --from-do`), making explicit that /do owns this mapping while the agent only emits the finding; (3) describes parsing as LLM-judgment based (no rigid schema); (4) Mid-Execution Amendment section updated to note `out-of-scope` hints route via Self-Amendment / /define --amend --from-do (same path as user-message-triggered amendments); (5) lifecycle ACs route through agent-emitted hints (github-pr-lifecycle), non-lifecycle ACs may emit hints directly; (6) explicit prohibition — merge-pr is NOT a supported action, /do does NOT invoke gh pr merge under any path; (7) explicit decoupling note — the agent emits findings, /do owns mapping findings to workflow (e.g., `out-of-scope` is the agent's finding; Self-Amendment is /do's workflow response). Threshold: no MEDIUM+ findings."
  ```

- [AC-3.4] Execution-mode files (`thorough.md`, `balanced.md`, `efficient.md`) document action-aware fix-cap semantics: only `fix-code` actions increment the per-phase fix-verify counter; `sleep`, `retrigger-ci`, `reply-thread`, `push-update`, `out-of-scope` are each named explicitly as non-counting actions. Per-AC `verify.timeout:` is the wall-clock cap that prevents lifecycle-AC runaway.
  ```yaml
  verify:
    method: codebase
    prompt: "In claude-plugins/manifest-dev/skills/do/references/execution-modes/{thorough,balanced,efficient}.md: verify each file's Fix-Verify Loops (or equivalent) section explicitly says the cap counts fix-code actions only; sleep, retrigger-ci, reply-thread, push-update, AND out-of-scope are each named (each action label appears in the text) as non-counting. `amend-manifest` must NOT appear as a hint label (it was renamed to `out-of-scope` per PG-7). Verify a cross-reference to per-AC verify.timeout: for lifecycle waits."
  ```
