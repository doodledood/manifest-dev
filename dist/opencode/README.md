...
- **`/run-ticket`** — receives one exact Ticket from a person or hosted trigger, claims it, invokes `/auto`, and writes DONE or ESCALATED evidence to the same item. It never scans the backlog or validates Auto; dispatch policy stays with the trigger.
