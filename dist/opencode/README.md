...
| Hooks | None shipped | None shipped | Use a durable goal-setting/continuation backstop with an auditable all-criteria-PASS completion contract for unattended turn continuation |
...
3. **No hook backstop for `/do`** — use a host-provided goal-setting/continuation backstop with an auditable all-criteria-PASS completion contract when you want the host CLI to keep `/do` running across turns: every manifest gate listed with fresh independent PASS evidence, not a summary claim. If no such capability is available, copy the contract the skill prints into your continuation mechanism.
