...
2. Invoke `manifest-dev:define --amend <manifest-path> --from-do`. **Multi-repo:** the manifest at `<manifest-path>` is shared across all repo PRs — every `/drive-tick` amends the same file, last-writer-wins (no locking; see `manifest-dev:define/references/MULTI_REPO.md` §f). If you notice a missed amendment after a collision, re-trigger the lost tick.
