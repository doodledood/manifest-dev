...
**A threshold can be wrong while the criterion is right.** A gate can describe its subject correctly and still sit at a bar costing more than it returns — findings keep landing, each real and each smaller, while every round re-verifies everything their subject touches. Convergence is not the test here; the repair loop is working, and that is the problem worth naming. It is a question about the gate rather than about the work, so it travels the same route gate text does: surface it with the evidence — what the recent rounds found, what another round would re-verify — and let the user hold the bar, lower it, or take the remainder as handover. Never lower it on the run's own reading. A bar the executor can move is not a bar, and grinding silently is the failure this exists to prevent: the goal is a verified result, not rounds.
