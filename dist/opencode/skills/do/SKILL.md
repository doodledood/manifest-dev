...
Sufficiency runs over gates whose premises stand. A PASS whose criterion misdescribes what it judges is not a settled PASS, whether its verifier reported that or execution surfaced it: route it to `/escalate` rather than completing on it, quoting the report or naming what execution showed. The user's ruling settles the premise question, not the ledger: affirming the text as written leaves the existing PASS standing, while amending it changes the gate's identity, so it returns to the ledger unverified and re-verifies like any other changed gate. That is the only thing a passing gate can still owe.
