...
Mid-/do user messages default to invoking `define` for amendment — the manifest is the source of truth, silent scope drift is worse than an extra amendment cycle. After the amendment returns, surface a one-line digest of what it assumed (the new or changed `(auto)`/ASM entries) so a user who steered and left can audit on return. Pure questions about the manifest or process are answered inline.
