...
emit the blocks below verbatim, substituting only `<manifest-path>`. Do not
...
invoke the figure-out skill with the task and `--autonomous`.
Invoke the define skill with `--autonomous`. Invoke the
just-do skill with the
