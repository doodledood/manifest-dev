...
Whether to split is one question; where the cut falls is another, and it is answered the same way whatever the input was. Every Shaped Ticket a split produces is a slice someone can exercise end-to-end on its own — put in front of its real use and seen working, not merely inspected as present. A Ticket cut along an implementation layer ("the schema", "the endpoints", "the screen") has nothing to run, so its definition of done falls back to checking that something exists and the picker cannot tell whether the work helped. Re-cut work that arrives framed in layers instead of passing the layers through; renaming a layer does not re-cut it. A Question Ticket is outside this — an investigation has no slice to run, and its done is the answer recorded.
