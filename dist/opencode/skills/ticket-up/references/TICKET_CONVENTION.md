...
That comparison needs an estimate worth trusting, and one shape of effort cannot supply one: a Ticket's payoff arrives long after the work, through a channel that mixes every Ticket's contribution with movement nobody controls, and most Tickets return almost nothing while a few carry the effort. Where all three hold — content libraries and discovery portfolios are the usual shape — a per-Ticket estimate is invented, and ranking two invented numbers ranks nothing. Such an effort states its own rule in its front file: a mix across the classes of work it does, each class ranked by something a picker can observe without predicting the outcome, and the mix revised against results read over batches rather than per Ticket. A picker meeting a qualifying effort that has not declared one picks by the default and says the effort needs a rule, rather than inventing a mix mid-pick. Per-Ticket comparison stays right everywhere else, and inside each declared class.
