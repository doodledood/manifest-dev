...
- **`docs/adr/CONVENTIONS.md` exists** → the project has its own ADR convention. It governs. Do not replace it, and follow it for anything written this run — then reconcile it, per *Reconcile what already exists* below.
...
- **A project context file already carries some of this wiring** → add what is missing and leave the rest alone, then reconcile its surfaces section the same way.
...
Compare the project's copy against **what the recipe below would emit today**, never against the raw reference: a correct copy already differs from its reference by the retitled heading, the replaced ownership paragraph, and any chapter the recipe drops, so comparing against the raw file reports those three every time and buries the real difference. Compare section by section and by meaning rather than line by line — a project that reworded or reflowed its copy has not fallen behind.
