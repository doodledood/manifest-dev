...
5. **Wire the project context file.** Emit the project-surfaces section from `references/NORTH_STAR_FORMAT.md`, adapted to what the project has — extend an existing section rather than duplicating one. It carries these four things, and nothing else, because this file is read on every session and every line here is a permanent cost:
   - **The North Star is resident, with its maintenance rule.** Import `NORTH_STAR.md` where the host supports imports, and state the instruction to read it at session start regardless; one line carries the asymmetry a contributor without manifest-dev needs — evidence may lower a line's state, positions change only by the owner's call, recorded as a decision record.
   - **The glossary is resident.** Import `CONTEXT.md` the same way. Write both as *the project context file*'s imports, not as a specific harness's filename.
