...
- Judgment findings are the distinct non-severity class the reference defines — author-answerable questions, synthesized to one per root **by the judgment subagent itself** (manifest mode skips the holistic pass, so the pass does its own collapsing), posted through the same batched `comment` review and voice. They are **not** criteria, so instead of the `ac=` extension they carry the `judgment` discriminator token (`<!-- review-pr judgment -->`) and are fingerprinted by finding substance like any other comment.
