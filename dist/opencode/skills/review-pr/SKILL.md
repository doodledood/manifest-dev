...
**Judgment pass (premise check).** No-manifest mode only here; manifest mode wires it via `references/MANIFEST_MODE.md`. The holistic-pass subagent above — which alone holds the PR description, conversation, and codebase direction the narrow-lens fleet lacks — also runs the judgment pass per `references/JUDGMENT_PASS.md`: the whole-PR question of whether the change earns its keep against the pain it solves, asked only when concrete evidence fires one of its triggers. Its altitude is the whole PR: it evaluates the full PR head on every pass, not the incremental reviewed range, so the range-bounding that scopes fleet findings to the reviewed delta (above) does **not** apply to it. Its findings are a **distinct non-severity class** (`{trigger, evidence, author-facing question}`), so the "drop Low severity" step above does **not** apply to them either — they surface whenever the reference's evidence-gate fires and the PR does not already cover the point, and are deduped, merged, and synthesized (one question per root) like any other finding; on loop passes, repeat-posting is suppressed by that already-covered / prune-against-prior-comments path, never by range-bounding. They post as author-answerable questions through the same voice, self-marker, and single batched `comment` review; the judgment pass never blocks, never submits `request_changes`, and never auto-approves.
