...
Invoke `auto` with the Ticket's complete prose anatomy, kind, definition of done, source reference, and relevant project context as the task. The Ticket bounds the work. Keep its identity available throughout the run so results return to the same store item.
