...
| invoke the `chat-surface` skill with: `text` — or the mode `--surface` resolves to. Invoke it; there is nothing here to read | always, before the first turn is shaped | the rendering contract that selects a turn's form, plus that destination's form vocabulary |
