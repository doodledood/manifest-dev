...
| invoke the `chat-surface` skill with: `text` — or, where `--surface` was passed, whichever skill and mode it resolves to instead — nothing here to read | always, before the first turn is shaped | the rendering contract that selects a turn's form, plus that destination's form vocabulary |
