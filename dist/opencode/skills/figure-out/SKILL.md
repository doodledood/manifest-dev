...
| the `chat-surface` skill — invoke it, don't read it | always, in the mode `--surface` names — `text` by default | the rendering contract that selects a turn's form, plus that destination's form vocabulary |
