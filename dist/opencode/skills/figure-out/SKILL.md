...
When the prototype renders as a page, invoke the design skill at the prototype weight it names. Where that skill is unavailable, hold its line by hand: pick the genre's register, keep the judged surface legible, leave the rest rough. A prototype that is words rather than a page — a breadboard, an outline, a command session — takes none of this.
