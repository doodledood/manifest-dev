...
5. **Repo identity confirmation.** When `cwd` is a git checkout AND `origin` is configured AND its owner/repo differ from the PR's base repo → note it (no halt): "Babysit target (`<pr-owner/repo>`) differs from cwd `origin` (`<cwd-owner/repo>`). Continuing — babysit doesn't require a local checkout."
