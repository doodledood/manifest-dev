...
**Name each gate's kind in its instructions.** Say whether the gate is a **Deterministic Gate**, whose verdict comes from a command or check returning the same outcome for the same artifact state, or a **Judgment Gate**, whose verdict is a model's judgment over an open finding space that a fresh evaluation resamples. `/do` reads the kind from the instructions to decide how the gate re-verifies — a Deterministic Gate in full every time, a Judgment Gate once in full and thereafter over its prior findings' repairs and the delta — and reads an unstated kind as Judgment, so state it wherever the wording could be taken either way. It goes in the instructions as prose: the verify block carries `instructions` and optional `phase`, and no other field. A user who wants one criterion re-sampled in full every round, at that cost, says so in that criterion's own instructions; the run-level `--exhaustive-verification` flag is the other way to buy the same thing for every gate at once.
...
The 13 review-code dimensions are: change-intent, code-bugs, contracts, type-safety (defect-finders, no LOW+); operational-readiness, code-design, code-maintainability, code-simplicity, code-testability, test-quality, docs, prose-value, context-file-adherence (advisory, no MEDIUM+). All thirteen are Judgment Gates; the nine advisory ones form the whole-change quality sweep and take a phase after the defect-finders and the project's mechanical gates, per `tasks/CODING.md`. The instructions already run inside `/do`'s selected execution envelope, so they tell that evaluator to **activate** the skill — never to spawn another agent, which would bypass the gate's PASS/FAIL/BLOCKED contract. For a non-review-code specialized check, name its skill the same way, e.g. *"Activate the check-pr skill. PR: …"*.
