...
Set a cheap, fast `verify.model`. Tracing each change back to the item that asked for it is mostly a lookup, and this gate runs more often than any other. Where it does need judgment — whether something nominally serving an item has outgrown the Appetite — the calibration below is what carries it, not model strength.

Carry the manifest's Problem, Appetite, and Out of bounds inline in its `verify.prompt`, along with what the work owes. A verifier sees only its own prompt, so a ceiling that refers to "the manifest" refers to something it cannot read. Carry the baseline with them, named as concretely as *Verifier prompt discipline* requires of any ref — the commit the run starts from, with everything at or before it taken as given. "What this run adds" is otherwise a boundary the verifier has no way to locate, and the ref it would reach for by default spans the very work it was told to inherit. Write the enumeration of what the work owes last, once the Deliverables and their criteria exist: filled in schema order it lists a set the manifest has not finished stating, and the gate then reads everything written after it as excess. Carry one carve-out too: work that discharges what a criterion required counts as required, including sweeping a changed rule into every copy and surface that holds it — other files stating it, distributed or generated copies, callers, and the READMEs that describe it. That work lands in files no criterion named, and a ceiling blind to it fails the sweep `/do` is separately obliged to perform.
