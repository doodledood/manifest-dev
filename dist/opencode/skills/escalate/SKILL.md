...
Also name any Process Guidance departed from, and why. This payload is a run-exit surface, so it carries the same obligation `/done` does: Process Guidance is advisory and nothing gates on it, so on a run that escalates rather than completing, this is where a departure surfaces.
