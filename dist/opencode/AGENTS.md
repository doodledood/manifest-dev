...
- **/chat-surface** — Live rendered chat view: the conversation lands in an auto-updating HTML page the user keeps open — their messages verbatim, agent responses rendered per point for minimum cognitive load (skimmable claims, charts, diagrams, decision cards). Activated directly or by `figure-out --surface chat-surface`; activation mid-session backfills the whole conversation. The terminal stays the wire — every failure is non-blocking.
