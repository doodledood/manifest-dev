...
- **/just-figure-out** — Goal-based investigation partner beside /figure-out. Presses a topic until understanding is shared and names a read — conclusion, confidence, evidence, what would overturn it — deciding for itself how to get there. Offers /just-define when the read implies work.
- **/just-define** — Goal-based Manifest builder beside /define. Encodes shared understanding into the same Manifest contract — schema followed exactly, every gate one text with a declared kind, the ceiling invariant verbatim — deciding for itself how to interview. Hands execution to /just-do.
- **/just-auto** — Goal-based autonomous chain beside /auto: just-figure-out --autonomous, then just-define, then just-do with the resulting Manifest path — no approval waits, no verification flags, the completion goal set by just-do after reading the Manifest (printed on hosts without a goal capability).
