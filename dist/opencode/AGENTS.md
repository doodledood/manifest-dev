...
- **/just-auto** — Goal-based autonomous chain beside /auto: just-figure-out --autonomous, then just-define, then just-do with the resulting Manifest path — no approval waits, no verification flags, one chain-spanning goal armed up front (printed-and-proceeded on hosts without a goal capability).
...
- **/design** — Builds or restyles a user-visible artifact — page, dashboard, document, deck, form, game — deciding purpose and register before code, tokens before markup, and verifying against rendered evidence.
