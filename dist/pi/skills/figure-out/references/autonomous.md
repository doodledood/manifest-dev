...
This is the standalone-run backstop. Suppress it only when you can clearly see `--autonomous` is chained under `/auto` (that wrapping entrypoint owns the goal-setting backstop); when in doubt, set or print it — a missing backstop costs more than a redundant one.
