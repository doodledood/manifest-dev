...
**Kind:** every dimension gate is a **Judgment Gate** — a reviewer's verdict over an open finding space, not a command outcome — so each declares the judgment kind, which is what tells `/do` how the gate re-verifies. The defect-finding dimensions above run alongside the project's mechanical gates below (typecheck, lint, test, format), which are Deterministic Gates — and an overlay task file may add one, as `BUG.md` adds `defect-class`. The advisory dimensions above are the whole-change quality sweep, and `/do` already spends their one full look once the mechanical and defect-finding gates are settled — no gate has to ask for that.
