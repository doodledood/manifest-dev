...
`Repos:` and `Repo:` are documentation for human and agent readers — they don't gate Harness-level Do.
...
The user is the coordinator: invoke `/manifest-do` or `/manifest-auto`, manage each repo's PR with whatever workflow they prefer, signal readiness in chat when cross-repo prerequisites land (BLOCKED criteria re-evaluate on the next `/manifest-do` invocation). The system supports this workflow but does not automate it.
