...
**A threshold can be wrong while the criterion is right.** A gate can describe its subject correctly and still sit at a bar costing more than it returns: findings keep landing, each real and each smaller, while every round re-verifies everything their subject touches. Convergence is not the test — the repair loop is working, which is what makes this hard to see from inside it. Read the bar as suspect once what the rounds turn up sits below the severity the gate was written to catch, or once re-verifying costs more than the remaining findings would change. That is a question about the gate rather than the work, so it goes to `/escalate` carrying what recent rounds found and what another round would re-verify. The user holds the bar or amends it: an amendment changes the gate's identity, so it returns to the ledger unverified and re-verifies like any other changed gate, while a bar left standing and unmet keeps the run off `/done` and exits it on the escalation path. Never lower it on the run's own reading — a bar the executor can move is not a bar.
