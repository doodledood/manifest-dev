...
Also name any Process Guidance departed from, and why. Escalation is a terminal path like `/done`, so it carries the same obligation: Process Guidance is advisory and nothing gates on it, which makes this the only place a departure surfaces on a run that never reaches a completion summary.
