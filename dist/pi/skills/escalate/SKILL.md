...
Also name any Process Guidance departed from, and why. Escalation is a terminal path like `/done`, so it carries the same obligation: Process Guidance is advisory and nothing gates on it, so on a run that escalates rather than completing, this payload is where a departure surfaces.
