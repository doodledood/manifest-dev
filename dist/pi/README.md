...
For unattended work, use a host goal-setting or continuation capability when available. The goal should say that the run is complete only after every Manifest AC/GI is listed in a gate ledger with fresh independent verifier PASS evidence, FAILs have been repaired and reverified, blockers are genuine, and no required verification is missing or stale. It should also say that unverified, FAIL, stale after a relevant change, BLOCKED/actionable, or escalation-pending gates are non-terminal. Do not accept self-attestation, "looks done", or a summary claim in place of verifier output.
