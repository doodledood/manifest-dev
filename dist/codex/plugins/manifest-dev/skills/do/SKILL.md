...
**A ceiling invariant is repaired by removing, not by widening.** It is the one gate with two apparent repairs — take the excess out, or grow the manifest until the excess traces to something — and only the first is available to the run. Amending scope so work already done becomes required is the executor deciding after the fact what it owed, which is what the gate exists to prevent, and it passes on the next round, so the loop reports success over the thing it was meant to catch. Where the FAIL genuinely exposes a scope gap rather than over-delivery, that is the user's question and routes to `/escalate`, the way a disputed gate text does.
