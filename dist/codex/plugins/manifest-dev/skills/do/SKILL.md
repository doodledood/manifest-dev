...
Sufficiency runs over gates whose premises stand, and over a change that is finished — a finding showing the change unfinished is repaired before `/done` even on an all-PASS ledger, per *A gate's threshold is the bar* below. A PASS whose criterion misdescribes what it judges is not a settled PASS: do not complete on it — it routes to `/escalate` per *Gate text is the exception* below, which owns that route. The user's ruling settles the premise question, not the ledger: affirming the text as written leaves the existing PASS standing, while amending it changes the gate's identity, so it returns to the ledger unverified and re-verifies like any other changed gate. That is the only thing a passing gate owes on its own premise.

### Acting on verdicts

Any actionable BLOCKED routes via `/escalate` — one whose note names something a person can do now. A BLOCKED that leaves only waiting reports as pending under a no-wait overlay; see *Caller overlays*. Without that overlay it waits and re-verifies on the normal cadence, bounded by *Runaway protection*. FAIL bodies carry findings or a natural-language hint — read them and act on what they say. When two verifiers of the same subject disagree about whether a rule holds, the disagreement is the finding: text two careful readers take oppositely is under-specified, whichever verdict is technically right. Sharpen the subject text rather than adopting the more convincing verdict — unless what they read oppositely is a gate's own criterion, which routes to `/escalate` per *Gate text is the exception* rather than being rewritten here.
