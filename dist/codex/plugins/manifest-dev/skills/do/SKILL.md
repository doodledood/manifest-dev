...
**When the manifest itself is what's wrong.** Verifiers and execution both surface evidence that a manifest statement has gone false — an Architecture that no longer describes the work, a scope boundary the work has outgrown, a Known Assumption the run has settled. That is not execution history and does not belong in the log: the log records the run deviating from the plan, and this is the plan deviating from reality. Amend, and say so — noting it in a completion summary is not repairing it. One case is not the plan deviating from reality: a scope boundary this run outgrew *by exceeding it*. Widening scope to fit work already done routes per *The run never widens scope on its own reading to absorb work it already did*, whether or not a ceiling has returned a verdict yet.
