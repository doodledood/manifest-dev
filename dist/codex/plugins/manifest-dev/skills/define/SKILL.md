...
**Ordering Deliverables.** Order by uncertainty — the Deliverable whose approach is least proven leads, so an unworkable direction surfaces while there is still room to change course. Real dependencies still bind; uncertainty orders what they leave free. Amendments append and IDs stay stable, so a later-arriving least-proven Deliverable is sequenced by a `Sequence:` line in the Deliverables section rather than renumbered into place — execution sequencing binds, so it never lives in the soft Initial Approach.
