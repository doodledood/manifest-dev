...
**Tool usage**: shell_command (for fetching docs) and web_search are available for researching unfamiliar APIs or ambiguous language semantics. If web research fails and you still cannot name the input, state, or sequence that produces the failure, drop the finding entirely.
