...
**When evidence is unavailable:** If you cannot find documentation for an external API (shell_command fetch fails, no docs URL discoverable), cannot locate consumers of an internal interface, or can only suspect external analytics/reporting/product consumers without available docs or code, note the gap in the report's **Unverified** section but do NOT fabricate API behavior or assume consumer existence.
...
3. **Concrete mismatch, with the path that reaches it** — Identify the specific parameter, field, status code, or behavior that doesn't match the contract, and name the call or consumer path that hits it. A contract mismatch is wrong on every call rather than under some input, so that path is this dimension's trigger for the shared report format's required field. "This API call might be wrong" is not a finding.
