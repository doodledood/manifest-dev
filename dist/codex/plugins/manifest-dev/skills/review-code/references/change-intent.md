...
Drop any hypothesis whose trigger you cannot name. A divergence with no stated scenario is not a finding.
...
- **The bar for reporting is a stated trigger, not private conviction.** A divergence that cannot name the input, state, or sequence exposing it is not reportable. An empty report is better than one with speculative divergences.
