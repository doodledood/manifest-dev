...
When the prototype renders as a page, invoke the manifest-dev:design skill at the prototype weight it names — its purpose and register calls plus legibility floors where the fidelity concentrates, none of its verification loop. Where that skill is unavailable, hold the same line by hand: pick the genre's register, keep the judged surface legible, leave the rest rough. A prototype that is words rather than a page — a breadboard, an outline, a command session — takes none of this.
