...
**Counterparty is a Slack channel or thread.** Operator launches in Claude Code; the team responds in Slack. **Arm `/loop` at a 2-minute default interval immediately after the first Slack post — never advance the turn without polling active.** Fall back to bash `sleep` at the same cadence only if `/loop` and cron tools are unavailable in the host environment. Each poll's read goes through a general-purpose agent that activates the `poll-slack` skill for a narrative of new messages since your cursor — keeps this session's context lean. Posting is inline from this session. Cursor and tree-state live in session memory; no scratch file.
