...
4. **Fork convention.** When the PR's `headRepository` differs from its `baseRepository` (cross-repo PR from a fork), babysit targets the **upstream repo where the PR lives** — i.e., the `baseRepository`'s owner/repo — not the fork. The agent invocation uses that canonical URL. Log the fork detection so the user can correct if they meant the fork copy.
