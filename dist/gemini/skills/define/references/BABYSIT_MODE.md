...
4. **Fork convention.** When the PR's `headRepository` differs from its `baseRepository` (cross-repo PR from a fork), babysit targets the **upstream repo where the PR lives** — i.e., the `baseRepository`'s owner/repo — not the fork. The agent invocation uses that canonical URL. Log the fork detection so the user can correct if they meant the fork copy. *Consequence*: any code-fix the agent suggests would target the PR's head branch, which lives on the fork — /do may not have push access. The agent's hint flags this as unrecoverable from within /do (caller should escalate to a human), deferring to the contributor to land the change themselves.
