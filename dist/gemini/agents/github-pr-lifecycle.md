...
- **Retrigger budget.** Default 10 retriggers per failing CI check per agent lifetime, overridable via steering. When relevant, name remaining budget so the caller can choose retrigger vs treat-as-real.
