...
3. **Context file**: operational `CLAUDE.md` references that mean "this CLI's context file" → `AGENTS.md` (instructions like "write to CLAUDE.md"; `define/tasks/CODING.md`'s project-preference/gate references are operational, so ALL of its CLAUDE.md mentions remap). Do NOT replace mentions of Claude Code's own file in comparative or research text.
...
4. **No Stop-hook backstop for `/do`** — preserve the skills' universal goal-setting backstop guidance for unattended continuation: use a host-provided goal-setting/continuation capability when available, otherwise surface the self-contained completion contract for the user to copy into their continuation mechanism. Do not drop the backstop or hardcode it to one host's primitive.
5. **Native `.claude/` compat overlap** — a user running OpenCode *inside a repo that itself contains manifest-dev-style `.claude/skills/`* gets those project skills too; project-local wins on name collisions. Not a sync concern, worth knowing when debugging duplicates.
