...
4. **No Stop-hook backstop for `/do`** — for unattended continuation use a `/goal` whose argument is a completion condition (all Acceptance Criteria and Global Invariants PASS), as the skills print. Keep the `/goal` backstop blocks intact (OpenCode supports Codex-style goal mode); do not strip them as Pi does.
