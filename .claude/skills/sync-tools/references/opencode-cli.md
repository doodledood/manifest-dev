...
4. **Context file**: operational `CLAUDE.md` references that mean "this CLI's context file" → `AGENTS.md` (instructions like "write to CLAUDE.md"; `define/tasks/CODING.md`'s project-preference/gate references are operational, so ALL of its CLAUDE.md mentions remap). Do NOT replace mentions of Claude Code's own file in comparative or research text.
