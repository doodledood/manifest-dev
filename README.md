...
**Multi-repo:** when a changeset spans repos, the same manifest covers all of it (Intent declares `Repos:`, deliverables tag `repo:`). `/do` navigates absolute paths from the map natively — no filter logic. `/tend-pr` and `/drive` run per-repo against the shared canonical `/tmp` manifest. Cross-repo gates the user explicitly triggers (e.g., post-deploy verification across services) use `method: deferred-auto` + `/verify --deferred`. See `claude-plugins/manifest-dev/skills/define/references/MULTI_REPO.md` for the full convention.
