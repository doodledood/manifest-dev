...
```bash
/babysit-pr [pr-url]          # infer or synthesize grounding, then run the lifecycle with a default journal
/babysit-pr --ci [pr-url]     # CI one-shot: advance state, push trusted fixes, exit on waiting
/babysit-pr --no-log [pr-url] # skip the default persisted journal
