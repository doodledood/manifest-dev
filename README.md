...
Run `/do` and `/auto` with a durable goal-setting or continuation backstop — the recommended way to run both unattended. Use the host's native goal-setting capability when it has one; otherwise copy the completion contract the skill prints into whatever continuation mechanism you use. The contract should be measurable and auditable: a complete manifest gate ledger with fresh independent PASS evidence for every AC/GI, not a bare command or summary claim. It should also carry the key constraints and blocker/stop condition so verification and fixes carry all the way through without you babysitting. Bare `/do` is fine for a quick single-turn run you're watching.
