...
**Execution.** After resolving the manifest path, invoke `manifest-dev:do` on it. For `--ci`, pass CI one-shot / no-wait context to `/do`. Do not print a follow-up command as the primary outcome; this skill owns the define→do chain. `/do` owns the execution contract: it reads the manifest, runs the lifecycle verifier by spawning a general-purpose agent that activates the `github-pr-lifecycle` skill, handles wait/retrigger/reply/sync findings, fixes in-scope code blockers when it can, and stops via `/done`, `/escalate`, or CI pending-summary when no-wait mode leaves only wait-shaped blockers.
