...
| `review-prompt` | Reviews LLM prompts against the `/prompt-engineering` skill's principles, leading with where each line came from. Reports issues without modifying files, tagging each `NEEDS_USER_INPUT` or `AUTO_FIXABLE` so an optimization loop can act on them. Use when reviewing prompt quality, auditing a prompt, or evaluating a system prompt. A gate body activates it when needed. |
