...
| `/prompt-engineering` | Create, update, or review an LLM prompt — system prompt, skill, or agent. State the goal, trust the model, add only what closes a real gap in natural behavior. Includes an opt-in empirical behavior-verification harness (`scripts/behavior_lab/`) for proving a wording change's effect with real captured traffic. |
