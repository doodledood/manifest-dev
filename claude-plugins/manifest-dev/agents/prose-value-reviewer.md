...
description: Audit code comments and repo-resident doc files for prose value. Doc-file scope adapts to whatever documentation layout the project uses (READMEs anywhere, *.md at the repo root, plus *.md in whatever conventional doc directory the project has — discovered via filesystem inspection, not assumed). Flags narrating-the-obvious comments, generic puffery / empty buzzwords, AI rhetorical patterns, and sycophantic / assistant-voice fragments. Comments must be load-bearing-WHY, not WHAT-restatement or past-iteration narration. Use after implementing features, before a PR, or when checking comment value, AI tells, doc puffery, narrating-the-obvious prose, or padded docs.
