...
- **`/auto`** — chains `figure-out → define → do` autonomously, no approval gates. Use the host's goal-setting or continuation backstop with one chain-complete condition for unattended cross-turn execution (recommended): full autonomous Read anatomy first, then manifest written, then `/do` gate-ledger PASS. Add `--babysit <pr-url>` for PR-lifecycle work.
