...
- **`/init-context`** — sets a repository up with the surfaces the workflow reads from: a North Star (`NORTH_STAR.md`, the project's standing strategy surface — who it's for, the promise, what winning means — every position carrying a dated state, seeded only from what the repo's own artifacts evidence and never invented), an ADR conventions file the project owns (self-contained, so a teammate running no tooling can use it), a glossary, and a project-context-file section that makes every session read all three and maintain them — the North Star under its update asymmetry: evidence may lower a position's state, positions change only by the owner's ruling, and a session notes when a decision it records sits outside what the Diagnosis names — a position also goes stale by drifting out of date while nothing contradicts it. Where the project has history, it seeds vocabulary and decision records from it — docs and pull-request discussion for reasoning, code and commits for what and when — and says so in the record when the reasoning could not be recovered rather than inventing one. Glossary candidates are ratified in one batch before anything is written, because that file is loaded on every session. Re-run it on a repository it already set up and it compares each project-owned copy against what it would emit today — the conventions files it wrote are forks that no sync updates, so it names what a current default carries that the copy does not and offers each difference one at a time, writing only what you accept. Mining runs by default; `--no-mine` installs the wiring alone. `/figure-out` offers it when a project has neither surface.
