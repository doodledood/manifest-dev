...
- **`/run-ticket`** — receives one exact Ticket from a person or hosted trigger, claims it, invokes `/auto`, and writes a terminal comment back to the same store item. DONE records evidence and closes the Ticket. ESCALATED records the blocker, attempts, preserved-work references, and human input needed, then leaves the same Ticket open and claimed for continuation. It never scans the backlog or validates Auto; dispatch policy stays with the trigger, so the skill remains portable across Claude Code, Codex, Pi, OpenCode, and future harnesses.
