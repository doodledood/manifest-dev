...
| `github-pr-lifecycle` | Steerable agent that inspects a GitHub PR's lifecycle state and returns a natural-language hint for /do to dispatch. Drives the PR toward a mergeable state; never invokes the merge button. Invoked by PR_LIFECYCLE.md's templated AC. |
