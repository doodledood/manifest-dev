...
When args contain `--team`, also load `references/team.md` and apply its overrides — the counterparty becomes a Slack channel or thread and the deliberation runs there, with the operator in Claude Code chat. `--team` supersedes `--autonomous`. Typically passed by the `figure-out-team` wrapper skill.
