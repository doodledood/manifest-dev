...
- When the read implies making something, state exactly what it will be and
  ...
  concrete artifact, before anything real is built. A draft rendered as a page
  goes through the manifest-dev:design skill at the prototype weight it names;
  where that skill is unavailable, pick the register and keep the judged
  surface legible by hand.
