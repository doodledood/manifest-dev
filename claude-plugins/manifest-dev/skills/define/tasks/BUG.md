...
|--------|-----------|-----------|
| Defect remediation | defect-class | no LOW+ |
...
It sits here rather than in the CODING base because it applies only where the change fixes a defect — it owns whether the repair provides justified protection against the demonstrated mechanism, including its unchanged enabling boundary.
...
- **Hand the mechanism forward** — Name the confirmed mechanism in the commit or PR description so the next debugger inherits it rather than re-deriving it. Adequacy of protection and affected-site accounting belong to the defect-class gate above
