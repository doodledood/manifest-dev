...
4. **Fork convention.** When the PR's `headRepository` differs from its `baseRepository` (cross-repo PR from a fork), babysit targets the **upstream repo where the PR lives** — i.e., the `baseRepository`'s owner/repo — not the fork. The agent invocation uses that canonical URL. Log the fork detection so the user can correct if they meant the fork copy. *Consequence*: any `fix-code` disposition the agent suggests would target the PR's head branch, which lives on the fork — /do may not have push access. In practice the agent suggests an `escalate` disposition when a write is needed against a fork-origin head branch, deferring to the contributor to land the change themselves.
...
- **Externally-closed PR mid-/do.** Detected by the agent at runtime, surfaced via FAIL with an `escalate` disposition. /do treats this as terminal — it does not attempt to reopen, and autonomous amendment to suppress the block is forbidden.
