...
**Safety-critical candidates.** Any candidate whose violation would be unsafe or irreversible — secrets and credentials, untrusted input, destructive or irreversible actions — becomes an Acceptance Criterion or Global Invariant, whatever its origin: a task-file Default, the interview, or a steering amendment. Sharpen it until a verifier can judge it against the best evidence available — the diff, the execution log, the run's own record — and never drop it for resisting verification, since dropping is the outcome this routing exists to prevent.
