...
## Correctness Rationale (Not Just Cost)

Manifest inlining is a correctness requirement, not only a cache-cost optimization — never skip it as a "the manifest is too short to cache" shortcut. A verifier that receives only its own `verify.prompt:` with no manifest context cannot resolve any cross-reference back to the manifest (e.g. a branch name fixed by Process Guidance, a Deliverable's framing).

The cache-staggering experiment (`tests/cache-experiment/`) captured this directly: on a manifest whose criteria referenced an unresolved placeholder (e.g. `<provider-branch>`, meant to resolve via a Process Guidance default), running the *same* prompt against the *same* commit five times with no manifest context produced five different outcomes — some repeats silently picked the right branch, one diffed the wrong ref entirely and returned a confidently incorrect verdict, and one returned no PASS/FAIL at all (it asked a clarifying question instead). With the manifest inlined, all five repeats resolved the same branch and reached the same verdict, every time. A criterion whose `verify.prompt:` never references anything outside itself won't show this failure mode — but authors can't assume that in general, since a manifest's Process Guidance and Deliverable framing are exactly the kind of context a verify prompt often leans on implicitly.

## Prompt Shape

Every verifier prompt has two parts, in this order:

1. **Shared block** — the full manifest content, inlined verbatim. Byte-identical across all agents within a `verify.model` group. It must literally begin with the manifest's real opening line (e.g. `# Definition: ...`).
...
2. Read the manifest exactly once at the start of the phase and write it to a frozen per-phase snapshot file (a plain copy of that read, not the live manifest path).
3. Compose every verifier prompt in the phase — including the priming call — against that snapshot's content as the shared block.
...
On Claude Code, the plugin ships a `PreToolUse` hook (`hooks/inline_manifest_hook.py`, registered declaratively via `hooks/hooks.json`) that removes the model from the manifest-reproduction path. Instead of pasting the snapshot's text into the prompt by hand, compose the shared block as the literal marker `{{INLINE_MANIFEST:<snapshot-path>}}` followed by the criterion's `verify.prompt:` tail. Before the Task tool call reaches the subagent, the hook reads `<snapshot-path>`, substitutes its exact bytes for the marker, and merges the result into `tool_input` (never a bare replacement — replacing would drop `description`/`subagent_type` and break the call). The hook fails closed: if the snapshot file can't be read, it denies the tool call with a reason naming the unreadable path rather than launching a verifier with a dangling marker.

This hook is a Claude-Code-only enhancement layer — it upgrades the guarantee from "instructed" to "mechanically enforced," but does not replace the instruction hardening above. On harnesses without a `PreToolUse`-equivalent mechanism (Pi, OpenCode), compose the shared block by pasting the snapshot's real text directly, following the same forbidden-placeholder rule.
