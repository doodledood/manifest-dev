...
**The run never widens scope to absorb work it already did.** Amending Out of bounds or Appetite so work already done becomes required is the executor deciding after the fact what it owed — the thing a ceiling invariant exists to prevent — and it hides itself, because the widened manifest passes on the next round and the loop reports success over exactly what it was meant to catch. This holds wherever the question arises, not only once a ceiling has failed: before any verdict, at a ceiling FAIL, and on any reading that the manifest's scope has gone stale. So a ceiling FAIL is repaired by removing the excess. Where the situation is genuinely a scope gap rather than over-delivery, that is the user's question and routes to `/escalate`, the way a disputed gate text does.
