...
- **`/figure-out`** — reach shared understanding of the problem. A peer that investigates before it claims, walks the decision tree, and holds positions under pushback. The conceptual core; call it directly when figuring it out IS the goal. Supports `--surface chat-surface` (the conversation renders live into an HTML page — the `/chat-surface` skill — while you keep typing in the terminal).
