...
| Plugin | What It Does |
|--------|--------------|
| [`manifest-dev`](./manifest-dev) | The core workflow: figure it out, encode what you'd accept, let it build and verify itself. `/figure-out` is the thinking partner; `/define` encodes that understanding into a Manifest; `/do` executes it and evaluates every gate under a run-level mode. `per-gate` is the independent default; `consolidated` is an opt-in mode for gate sets that are large and individually cheap, and `self` an opt-in lower-assurance one. The manifest is the canonical source of truth for the PR/branch — feedback during `/do` or after `/done` defaults to amending it. `/init-context` sets a repository up with the surfaces the workflow reads from — project-owned ADR conventions, a glossary, and the context-file wiring — seeding them from the project's own history where there is any. Multi-CLI distribution (OpenCode, Codex CLI, and a Pi package target). Running `/do` with a durable goal-setting/continuation backstop whose contract is auditable all-criteria-PASS — every manifest gate in a ledger with fresh mode-appropriate evidence — is the recommended unattended form; the backstop keeps the run alive across turns. |
...
- **`/ticket-up` and `/next-ticket`** — split a finished Manifest into self-sufficient tickets anyone can pick up (your shared tracker, GitHub Issues, or files in the repo, on a venue asked once per project), and pull the top ready ticket back out with the reason it leads — across several efforts without asking you which one, claiming it so parallel sessions each get a different one.
