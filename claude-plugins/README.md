...
- **`/figure-out`** — reach shared understanding of the problem. A peer that investigates before it claims, walks the decision tree, and holds positions under pushback. The conceptual core; call it directly when figuring it out IS the goal. Supports `--canvas` (a live browser-rendered map of where the session has reached, annotatable and handed back in one paste).
