...
- **`/ticket-up`, `/next-ticket`, and `/run-ticket`** — author self-sufficient Tickets in the project's store, claim and present the top ready one, or run one exact Ticket through `auto` and write DONE or ESCALATED evidence back to the same item. A Manifest stays one coherent Ticket by default; explicit delegation can split it on Deliverable boundaries.
