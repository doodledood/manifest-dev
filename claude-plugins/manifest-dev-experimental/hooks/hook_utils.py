#!/usr/bin/env python3
"""
Shared utilities for manifest-dev-experimental hooks.

Contains transcript parsing for skill invocation detection.

Namespace-scoped: only `manifest-dev-experimental:<skill>` invocations register.
...
PLUGIN_NAMESPACE = "manifest-dev-experimental"
...
def get_skill_call_args(line_data: dict[str, Any], skill_name: str) -> str | None:
    ...
    Matches ONLY the namespaced form `manifest-dev-experimental:<skill>`.
    """
...
def was_skill_invoked(line_data: dict[str, Any], skill_name: str) -> bool:
    ...
    Detects invocation patterns scoped to `manifest-dev-experimental:<skill>`:
    ...
    3. User command-name tag (/manifest-dev-experimental:<skill>)
    """
...
def _is_user_skill_invocation(line_data: dict[str, Any], skill_name: str) -> bool:
    ...
    # in the "Base directory" path (manifest-dev-experimental/skills/<name>/).
