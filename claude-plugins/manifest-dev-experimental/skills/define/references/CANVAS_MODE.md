...
2. **Invoked via `/auto`.** /auto runs autonomously without a human reviewer in the loop. The canvas's value is live human review; without a reviewer present, it's wasted tokens. Silent skip when /auto is the caller. (Experimental has no `--interview` flag — the autonomous pathway IS /auto, not a flag value.)
