...
*(Note: experimental's manifest schema doesn't carry `Mode:` or `Interview:` fields — there is one mode for both. The original main-plugin BABYSIT_MODE seeded these into Intent; experimental babysit skips them.)*
