...
- **Announce before side effects.** In inline-fallback only: print the line from `references/fallback-inline.md` §Announce Protocol before any side-effecting operation — branch creation, commit, push, PR open, lock creation, log initialization. Read-only pre-flight checks may precede it; side-effecting ones may not.
