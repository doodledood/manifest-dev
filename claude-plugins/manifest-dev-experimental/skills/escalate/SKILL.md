...
- **Self-Amendment** — user/reviewer feedback contradicts or extends the manifest. Quoted trigger. Re-entry depends on source: from /do or /verify → autonomous fast path (`/define --amend <path> --from-do`, /do resumes); after /done → two-step chain (`define --amend`, then `do --scope <affected>`), both steps mandatory.
