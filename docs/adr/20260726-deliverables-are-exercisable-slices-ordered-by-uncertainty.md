...
List order *is* the execution order — stating that is what makes the ordering rule mean anything to `/do`, and it is the foundation the guidance rests on. Position and ID are therefore independent: an amendment inserts a new Deliverable where uncertainty puts it rather than always last, taking a fresh ID without renumbering the others, so `/do`'s gate ledger keeps its stable keys while the uncertainty ordering survives amendment. There is no separate sequencing field. The order is soft in the same sense the Initial Approach is: `/do` may resequence when execution changes what the order was built on — a real dependency the ordering missed, or a shift in which Deliverable is now least proven — recording the deviation. The manifest therefore carries a second non-binding element alongside the Initial Approach, and both `/do` and the emitted schema name the pair. Per `20260726-only-gates-bind-process-guidance-is-advisory`, the binding layer is Acceptance Criteria and Global Invariants alone.
