...
2. **Upstream triage of load-bearing unknowns.** A leftover gap whose impact-if-wrong is a failed *approach* (the run would re-plan, not merely re-do work) is un-assumable: `/define` either bounces it to figure-out for resolution or settles it deliberately as a decided element — dictating a good-enough patch on the record, Shape Up's hole-patching move. Ordinary defaults continue to flow to ASM entries unchanged.
