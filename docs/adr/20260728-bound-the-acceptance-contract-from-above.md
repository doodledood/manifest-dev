...
- **Intent carried inline in its `verify.prompt`.** A verifier sees only its own prompt, so a ceiling referring to "the manifest" refers to something it cannot read. This makes it the first gate holding a copy of the manifest's own content — the Intent fields and the enumeration of what the work owes — so amendment must refresh both. A stale copy judges the work against the scope the amendment replaced and fails exactly what it required, and an amendment adding work inside the existing Appetite moves the enumeration while leaving Intent untouched. `/define`'s frame-reconciliation rule names it for that reason.
