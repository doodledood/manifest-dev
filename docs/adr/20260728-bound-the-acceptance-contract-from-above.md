...
**From above — `/define` emits a ceiling Global Invariant on every manifest.** It fails a change that carries work no Acceptance Criterion, Global Invariant, or Deliverable required, and work that nominally serves one of those while far exceeding the surface the Appetite allows. Its subject is what the run adds rather than the artifact the run started from, so a manifest synthesized over existing work — `--babysit` over a pull request — takes that work as given. That boundary is stated as categories of inherited work rather than as a baseline commit: a pinned ref fixes at authoring time a boundary that keeps moving as the run resumes, as the base branch is merged in, and it means nothing for a prose deliverable or a manifest spanning several repositories. Four properties are specified with it:
...
- **A cheap, fast `verify.model` where the host offers one.** Tracing each change back to the item that asked for it is mostly a lookup, and this gate runs more often than any other. Per-verifier model selection is not available on every distribution, so the instruction is conditional and the field is left to inherit where it is not; the judgment the gate does need is carried by its calibration rather than by model strength, which is what keeps it affordable either way.
- **Intent carried inline in its `verify.prompt`,** with carve-outs for work that is inherited rather than added and for work discharging what a criterion required. A verifier sees only its own prompt, so a ceiling referring to "the manifest" refers to something it cannot read. This makes it the first gate holding a copy of the manifest's own content — the Intent fields and the enumeration of what the work owes — so amendment must refresh both. A stale copy judges the work against the scope the amendment replaced and fails exactly what it required, and an amendment adding work inside the existing Appetite moves the enumeration while leaving Intent untouched. `/define`'s frame-reconciliation rule names it for that reason.

`/do`'s threshold passage is re-hosted into a rule and its exception, keeping the existing restraint prose intact and adding the ceiling as a second edge to the cost that passage already named. The prose is not replaced by the gate: `/do` runs on manifests that carry no ceiling — hand-written ones, and every manifest predating this change — so it still has to stand on its own there. `/do` also states the ceiling's repair direction, because it is the first gate with two apparent ones: a ceiling FAIL is repaired by removing the excess, never by widening the manifest until the excess traces to something. The second route would let an executor decide after the fact what it owed — the exact disposition the gate is built against — and it would pass on the following round, so the loop would report success over the thing it was meant to catch. A FAIL that genuinely exposes a scope gap is a question for the user.
...
- Requiring a stated trigger will drop some true findings whose mechanism a reviewer sensed but could not name. That is the intended trade — an empty report beats a false positive on a defect-finder — but it is a genuine loss, not a free one. Taste-level type hygiene is the concrete case: it names no trigger, so `type-safety` no longer reports it as a type hole. It moves to `code-maintainability` under Consistency, where it is advisory rather than blocking — which is the right home for it, but a lower bar than where it sat before.
