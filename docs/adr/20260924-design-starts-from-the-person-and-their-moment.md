...
Four rounds of blind comparisons tested this. Each round generated the same briefs under different prompts in isolated contexts, and rounds 1 to 3 also included a no-skill baseline. The owner ranked the results without knowing which prompt produced which page:
...
2. **Round 2:** jazz club, payroll software, and hospice volunteering. A three-sentence prompt, the subject-first prompt, beat the no-skill baseline on all three. It asks the model to make the audience feel something and to find the visual idea in the subject's materials, rituals, and artifacts. It then asks for palette, type, and texture taken from the subject rather than a house style.
...
The prompt asks who is present, what they came to do, and how long they will stay. It asks for the feeling that moment wants, at the strength it wants, unless the request names a strength. It takes the visual idea, palette, type, and texture from the subject and the audience's world rather than from a house style. Explicit requirements and an existing design system outrank it, and refinement keeps what works and changes what the feedback names. Artifact type is not modeled separately; it stands in for the moment, and the moment is what the prompt names.

Delete the five catalogs. `review-design` keeps ownership of its standards and HTML checker as before, and still consults `design` for art direction, which the brief and any established design system override.

## Alternatives Considered

- **Keep the catalogs:** tied with the no-skill baseline while taking roughly twice the time, won only inside its own domain, and pulled unrelated subjects toward one look.
- **An audience, goal, feeling, and guardrail statement:** came last on every brief it was tried on; asking the model to reason about feelings and their limits likely made it cautious.
...
- Tools used for hours and one-time pieces meant to delight are covered by the same principle.
