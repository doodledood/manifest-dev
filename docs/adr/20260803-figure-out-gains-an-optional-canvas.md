...
- **Refresh uses the canvas's guarded reload path.** After data replacement, the user presses the visible **R** shortcut. An already-loaded `file://` page cannot detect replacement without polling or a server; the explicit step preserves the no-dependency boundary and avoids a disruptive reload loop. When storage is unavailable and notes exist only in memory, **R** opens the return bundle instead, and a browser-native reload triggers the page's unload warning.
