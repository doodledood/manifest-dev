...
- **Refresh uses the browser's explicit reload path.** After data replacement, the user presses the canvas's visible **R** shortcut or the browser reload command. An already-loaded `file://` page cannot detect replacement without polling or a server; the explicit step preserves the no-dependency boundary and avoids a disruptive reload loop.
